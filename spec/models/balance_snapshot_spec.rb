...
RSpec.describe BalanceSnapshot, type: :model do
  ...
  describe "scopes" do
    ...
    it ".fine returns only fine granularity" do
      expect(BalanceSnapshot.fine).to contain_exactly(fine_snap)
    end

    it ".hourly returns only hourly granularity" do
      expect(BalanceSnapshot.hourly).to contain_exactly(hourly_snap)
    end

    it ".daily returns only daily granularity" do
      expect(BalanceSnapshot.daily).to contain_exactly(daily_snap)
    end

    it ".for_period filters by snapshot_at range" do
      expect(BalanceSnapshot.for_period(3.hours.ago, Time.current))
