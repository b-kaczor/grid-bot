...
RSpec.describe SnapshotRetentionJob, type: :job do
  ...
  describe "#perform" do
    ...
    context "with fine snapshots in the 7-30 day window" do
      let!(:snapshot_8d) { create(:balance_snapshot, bot:, granularity: "fine", snapshot_at: 8.days.ago.beginning_of_hour + 2.minutes) }
      let!(:snapshot_8d_2) { create(:balance_snapshot, bot:, granularity: "fine", snapshot_at: 8.days.ago.beginning_of_hour + 30.minutes) }
    ...
    context "with hourly snapshots older than 30 days" do
      let!(:hourly_old_1) { create(:balance_snapshot, bot:, granularity: "hourly", snapshot_at: 35.days.ago.beginning_of_day + 6.hours) }
      let!(:hourly_old_2) { create(:balance_snapshot, bot:, granularity: "hourly", snapshot_at: 35.days.ago.beginning_of_day + 18.hours) }
    ...
    context "with existing aggregates (idempotency)" do
      let!(:fine_old) { create(:balance_snapshot, bot:, granularity: "fine", snapshot_at: 10.days.ago.beginning_of_hour + 5.minutes) }
      let!(:existing_hourly) { create(:balance_snapshot, bot:, granularity: "hourly", snapshot_at: 10.days.ago.beginning_of_hour) }
